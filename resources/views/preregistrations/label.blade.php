<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Etiqueta - {{ $preregistration->warehouse_code }} - BCH Tracking</title>
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body {
            font-family: 'Helvetica Neue', Arial, sans-serif;
            background: #f3f4f6;
            padding: 16px;
        }
        /* Contenedor etiqueta: 4" de ancho; altura compacta para una sola hoja */
        .label-sheet {
            width: 4in;
            min-height: 0;
            max-width: 100%;
            margin: 0 auto;
            background: white;
            border: 2px solid #111;
            border-radius: 8px;
            padding: 6px 10px;
            box-shadow: 0 2px 8px rgba(0,0,0,0.1);
        }
        .label-sheet .label-header {
            display: flex;
            align-items: center;
            justify-content: space-between;
            gap: 4px;
            margin-bottom: 4px;
            padding-bottom: 3px;
            border-bottom: 1px solid #1e40af;
        }
        .label-sheet .label-header .company-block {
            margin: 0;
            padding: 0;
            border: none;
        }
        .label-sheet .label-header .company {
            font-size: 11px;
            font-weight: 700;
            color: #1e40af;
            letter-spacing: 0.02em;
            margin: 0;
            padding: 0;
            border: none;
        }
        .label-sheet .label-header .company-address {
            font-size: 8px;
            color: #374151;
            margin-top: 1px;
            letter-spacing: 0.02em;
        }
        .label-sheet .label-header .company-city {
            font-size: 8px;
            color: #374151;
            margin-top: 0;
            letter-spacing: 0.02em;
        }
        .label-sheet .label-header .agency-logo-wrap {
            min-height: 36px;
            max-width: 130px;
            display: flex;
            align-items: center;
            justify-content: flex-end;
            background: transparent;
            padding: 0;
        }
        .label-sheet .label-header .agency-logo {
            max-height: 36px;
            max-width: 120px;
            width: auto;
            height: auto;
            object-fit: contain;
            object-position: right center;
            display: block;
            background: transparent;
            mix-blend-mode: multiply;
        }
        .label-sheet .label-header .agency-right {
            display: flex;
            flex-direction: column;
            align-items: flex-end;
            gap: 4px;
        }
        .label-sheet .label-header .agency-name-fallback {
            font-size: 10px;
            font-weight: 600;
            color: #1e40af;
            text-align: right;
            max-width: 140px;
        }
        .label-sheet .label-header .agency-name-below {
            font-size: 8px;
            font-weight: 600;
            color: #1e40af;
            text-align: right;
            max-width: 140px;
            line-height: 1.15;
        }
        .label-sheet .barcode-section {
            margin-top: 6px;
        }
        .label-sheet .warehouse-code {
            font-size: 28px;
            font-weight: 800;
            letter-spacing: 0.12em;
            text-align: center;
            margin: 4px 0;
            font-family: 'Helvetica Neue', Arial, sans-serif;
            font-variant-numeric: tabular-nums;
            color: #111;
        }
        .label-sheet .barcode-wrap {
            text-align: center;
            margin: 3px 0 6px;
            max-width: 100%;
            overflow: hidden;
        }
        .label-sheet .barcode-wrap canvas {
            max-width: 100%;
            height: auto !important;
        }
        /* Títulos (labels) en negro: mayor contraste */
        .label-sheet .field {
            font-size: 9px;
            color: #111827;
            text-transform: uppercase;
            letter-spacing: 0.06em;
            font-weight: 800;
            margin-top: 0;
        }
        .label-sheet .value {
            font-size: 12px;
            font-weight: 600;
            color: #111;
            margin-top: 1px;
        }
        .label-sheet .value.value-sm { font-size: 10px; font-weight: 600; }
        .label-sheet .value.value-code { font-family: ui-monospace, 'Courier New', monospace; font-size: 11px; letter-spacing: 0.06em; }
        .label-sheet .value.value-destination { font-size: 12px; font-weight: 800; }
        .label-sheet .value.label-tracking {
            font-family: 'Helvetica Neue', Arial, sans-serif;
            font-variant-numeric: tabular-nums;
            font-size: 11px;
        }
        .label-sheet .value.label-service {
            font-size: 15px;
            font-weight: 800;
            letter-spacing: 0.03em;
        }
        .label-sheet .value.label-service-air {
            color: #0f766e;
        }
        .label-sheet .value.label-service-sea {
            color: #1e40af;
        }
        .label-sheet .reception-note {
            margin-top: 6px;
            padding: 5px 8px;
            background: #eff6ff;
            border: 1px solid #93c5fd;
            border-radius: 4px;
        }
        .label-sheet .reception-note .title {
            font-size: 8px;
            text-transform: uppercase;
            letter-spacing: 0.05em;
            color: #111827;
            font-weight: 700;
            margin-bottom: 2px;
        }
        .label-sheet .reception-note .datetime {
            font-size: 12px;
            font-weight: 700;
            color: #1e3a8a;
        }

        /* Layout tipo etiqueta logística (compacto para una hoja) */
        .label-sheet .kv {
            padding: 3px 0;
            border-bottom: 1px solid #e5e7eb;
        }
        .label-sheet .kv:first-of-type { padding-top: 2px; }
        .label-sheet .kv:last-of-type { border-bottom: none; }
        .label-sheet .kv.kv-destination {
            background: #f8fafc;
            border: 1px solid #e5e7eb;
            border-left: 4px solid #111827;
            border-radius: 4px;
            padding: 4px 7px;
            margin-top: 3px;
        }
        .label-sheet .kv.kv-3col {
            display: grid;
            grid-template-columns: 1.1fr 0.9fr 1fr;
            gap: 4px;
            align-items: start;
            padding: 4px 0;
        }
        .label-sheet .kv.kv-3col .kv-col { min-width: 0; }
        .label-sheet .kv.kv-3col .value { margin-top: 2px; }
        .label-sheet .kv .value { margin-top: 2px; }
        .label-sheet .kv.kv-3col { border-bottom: 1px solid #e5e7eb; }
        .label-sheet .kv + .barcode-section { margin-top: 4px; }

        /* Mejor lectura del código grande sin “lavarse” */
        .label-sheet .warehouse-code { text-shadow: 0 1px 0 rgba(0,0,0,0.08); }
        .no-print {
            text-align: center;
            margin-bottom: 16px;
        }
        .no-print-hint {
            font-size: 13px;
            color: #6b7280;
            margin-top: 8px;
            margin-bottom: 0;
        }
        .no-print button {
            background: #2563eb;
            color: white;
            border: none;
            padding: 12px 24px;
            font-size: 16px;
            border-radius: 8px;
            cursor: pointer;
            font-weight: 600;
        }
        .no-print button:hover { background: #1d4ed8; }
        .no-print a {
            display: inline-block;
            margin-left: 12px;
            color: #4b5563;
            font-size: 14px;
        }

        @media print {
            @page { size: 4in 6in; margin: 0; }
            body { background: white; padding: 0; }
            .no-print { display: none !important; }
            .label-sheet {
                width: 4in;
                min-height: 0;
                max-width: none;
                margin: 0;
                border: 1px solid #000;
                box-shadow: none;
                page-break-inside: avoid;
                zoom: 0.92;
            }
        }
    </style>
</head>
<body>
    @if(!request('embed'))
    <div class="no-print">
        @if(session('success'))
        <p style="margin-bottom: 12px; padding: 10px; background: #d1fae5; color: #065f46; border-radius: 6px; font-size: 14px;">{{ session('success') }}</p>
        @endif
        @if(session('warning'))
        <p style="margin-bottom: 12px; padding: 10px; background: #fef3c7; color: #92400e; border-radius: 6px; font-size: 14px;">{{ session('warning') }}</p>
        @endif
        <button type="button" onclick="window.print();" class="no-print-btn">🖨️ Imprimir etiqueta</button>
        <p class="no-print-hint">Seleccione la impresora de etiquetas en el cuadro de impresión.</p>
        @if(!empty($dropoffNextStep) && !empty($dropoffTotal))
        <p style="margin-top: 14px;">
            <a href="{{ route('preregistrations.create') }}" style="display: inline-block; padding: 8px 14px; background: #0d9488; color: #fff; border-radius: 6px; font-weight: 600; text-decoration: none;">Continuar con el siguiente bulto ({{ $dropoffNextStep }}/{{ $dropoffTotal }})</a>
        </p>
        <p style="margin-top: 6px; font-size: 13px; color: #6b7280;">Después de imprimir esta etiqueta, completa los datos del bulto {{ $dropoffNextStep }}.</p>
        @endif
        <p style="margin-top: 12px;"><a href="{{ route('preregistrations.show', $preregistration->id) }}">← Volver al preregistro</a></p>
    </div>
    @else
    <style>.no-print { display: none !important; } body { padding: 8px; background: #f9fafb; }</style>
    @endif

    @include('preregistrations.partials.label-sheet', compact('preregistration'))

    <script src="https://cdn.jsdelivr.net/npm/jsbarcode@3.11.6/dist/JsBarcode.all.min.js"></script>
    <script>
        document.querySelectorAll('.barcode-canvas').forEach(function(el) {
            if (el.dataset.barcode) JsBarcode(el, el.dataset.barcode, {
                format: 'CODE128',
                width: 2,
                height: 34,
                displayValue: true,
                fontSize: 11,
                margin: 2
            });
        });
    </script>
</body>
</html>
